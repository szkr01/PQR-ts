...
export type QRPointTypeValue = typeof QRPointType[keyof typeof QRPointType];
...
export function encode(
  text: string,
  options: { ecc?: 'low' | 'medium' | 'quartile' | 'high' } = {}
): [boolean[][], QRPointTypeValue[][]] {
  const eccMapping = {
    'L': 'low',
    'M': 'medium',
    'Q': 'quartile',
  ...
  if (mappedOptions.ecc) {
    mappedOptions.ecc = eccMapping[mappedOptions.ecc as keyof typeof eccMapping] || mappedOptions.ecc;
  }
  ...
  let typeTable: QRPointTypeValue[][] = new Array(nCount);
        ...
        for (let r = -2; r <= 2; r++) {
          for (let c = -2; c <= 2; c++) {
            if (
              ...
              typeTable[position[i] + r][position[j] + c] = QRPointType.ALIGN_OTHER;
            }
          }
        }
      }
